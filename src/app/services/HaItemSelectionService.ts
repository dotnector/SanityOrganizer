 ...
 * This is using in the organizer table that allows adding HaItems to folders.
 */
export class HaItemSelectionService {
  public onRowClick(
  ...
  public onCheckboxToggle(selectedIds: Set<string>, itemKey: string, checked: boolean): Set<string> {
    const next = new Set(selectedIds);
    if (checked) {
      next.add(itemKey);
    } else {
