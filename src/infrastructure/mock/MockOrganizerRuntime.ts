...
export class MockOrganizerRuntime implements OrganizerRuntime {
  ...
  public async loadHaItemCatalog(): Promise<HaItemCatalog> {
    Logger.debug(`${MockOrganizerRuntime.LOG_PREFIX} loadHaItemCatalog:start`);
    Logger.debug(`${MockOrganizerRuntime.LOG_PREFIX} loadHaItemCatalog:success`, {
  ...
  public async loadState(): Promise<OrganizerState> {
    Logger.debug(`${MockOrganizerRuntime.LOG_PREFIX} loadState:start`);
    try {
      const state = this.stateStore.load(() => this.stateSeeder.createInitialState(this.catalog));
      Logger.debug(`${MockOrganizerRuntime.LOG_PREFIX} loadState:success`, {
        folderCount: Object.keys(state.folders).length,
        rootFolderCount: state.rootFolderIds.length,
      });
      return state;
    } catch (error) {
      Logger.error(`${MockOrganizerRuntime.LOG_PREFIX} loadState:error`, error);
      throw error;
    }
  }

  public async saveState(state: OrganizerState): Promise<void> {
    Logger.debug(`${MockOrganizerRuntime.LOG_PREFIX} saveState:start`, {
      folderCount: Object.keys(state.folders).length,
      rootFolderCount: state.rootFolderIds.length,
    });
    try {
      this.stateStore.save(state);
      Logger.debug(`${MockOrganizerRuntime.LOG_PREFIX} saveState:success`);
    } catch (error) {
      Logger.error(`${MockOrganizerRuntime.LOG_PREFIX} saveState:error`, error);
      throw error;
    }
  }
}
